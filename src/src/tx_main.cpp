...
void LEDTest()
{
    ...
    while (true)
    {
        digitalWrite(PB5, HIGH); // LED ON
        delay(500);
        digitalWrite(PB5, LOW); // LED OFF
        delay(500);
    }
}
...
void SetRFLinkRate(uint8_t index) // Set speed of RF link
{
    ...
    if ((ModParams == ExpressLRS_currAirRate_Modparams) && (RFperf == ExpressLRS_currAirRate_RFperfParams) && (invertIQ == Radio.IQinverted) && (OtaSwitchModeCurrent == newSwitchMode))
        return;
