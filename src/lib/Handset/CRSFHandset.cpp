...
void CRSFHandset::Begin()
{
    ...
    // CRSFHandset::Port.setRxInvert(true);
    // CRSFHandset::Port.setTxInvert(true);
    DBGLN("RC TX: %d", GPIO_PIN_RCSIGNAL_TX);
    DBGLN("RC RX: %d", GPIO_PIN_RCSIGNAL_RX);
    // DBGLN("PCLK2=%lu", HAL_RCC_GetPCLK2Freq());
    ...
    //  g.Pin = GPIO_PIN_6;
    //  g.Mode = GPIO_MODE_AF_PP;            // Alternate Function Push-Pull
    //  g.Pull = GPIO_NOPULL;                // No pull-up/down
    //  g.Speed = GPIO_SPEED_FREQ_VERY_HIGH; // Fast edges for 400–420k baud
    //  g.Alternate = GPIO_AF7_USART1;       // AF7 = USART1_TX
    ...
    USART2->CR1 |= USART_CR1_UE;
#elif defined(TARGET_ELRSSX1280TX_PCB)
    LL_GPIO_SetPinPull(GPIOB, GPIO_PIN_6, LL_GPIO_PULL_NO); // default is PULLUP
    USART1->CR1 &= ~USART_CR1_UE;
    // USART1->CR3 |= USART_CR3_HDSEL;
    //  USART1->CR1 |= USART_CR1_RE;  // keep receiver enabled
    //  USART1->CR1 &= ~USART_CR1_TE; // TX driver off (Hi-Z) at idle
    ...
    // GPIOB->PUPDR &= ~(0x3u << (6 * 2)); // PB6 PUPD[13:12] = 00 (NO PULL)
...
bool CRSFHandset::processInternalCrsfPackage(uint8_t *package)
{
    ...
    // Enter Binding Mode
    if (packetType == CRSF_FRAMETYPE_COMMAND && header->frame_size >= 6 // official CRSF is 7 bytes with two CRCs
        && header->dest_addr == CRSF_ADDRESS_CRSF_TRANSMITTER && header->orig_addr == CRSF_ADDRESS_RADIO_TRANSMITTER && header->payload[0] == CRSF_COMMAND_SUBCMD_RX && header->payload[1] == CRSF_COMMAND_SUBCMD_RX_BIND)
    {
        if (OnBindingCommand)
            OnBindingCommand();
        return true;
...
void CRSFHandset::duplex_set_RX() const
{
    ...
    // {
    // } // ensure last bit out
    // USART1->CR1 &= ~USART_CR1_TE; // Hi-Z
    // Keep core state sane (optional):
    CRSFHandset::Port.enableHalfDuplexRx();
    // GPIOB->PUPDR &= ~(0x3u << (6 * 2)); // PB6: NO PULL
#endif
}
...
void CRSFHandset::duplex_set_TX() const
{
    ...
    // USART1->CR1 |= USART_CR1_TE; // enable TX driver
    // __DSB();                     // fence so write hits before bytes
#endif
}
